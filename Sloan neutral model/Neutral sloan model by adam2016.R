...
sncm.fit <- function(spp, pool=NULL, stats=TRUE, taxon=NULL){
	...
	m.mle <- mle(sncm.LL, start=list(m=0.1, sigma=0.1),method = "Nelder-Mead", nobs=length(p))
	...
	bino.mle <- mle(bino.LL, start=list(mu=0, sigma=0.1), method="Nelder-Mead", nobs=length(p))
